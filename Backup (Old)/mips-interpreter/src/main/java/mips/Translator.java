...
public class Translator {
				...
				switch ( ins.opCode ) {
						...
						case LABEL:
								if(ins.operands[0].toString().compareTo("return") == 0) {
										...
										mips_ins = ins.operands[0] + "_" + current_func.name + ":\n";
								}
								break;
						default:
								break;
		...
		public String add(IRInstruction tigerir) {
				...
				if (is_var(tigerir.operands[1]) & is_var(tigerir.operands[2])) {
						...
						mips = mips + "wtf " + Arrays.asList(tigerir.operands) + "\n";
				}
				return mips;
		}
		...
		public String brlt(IRInstruction tigerir) {
				String mips = "";
				if(is_var(tigerir.operands[0]) & is_var(tigerir.operands[1])) {
