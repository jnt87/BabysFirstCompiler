...
LONG RegMan_CreateKey(HKEY hKey,
										   LPCTSTR lpstrKeyName,
										   PHKEY phKeyDest)
{
	DWORD	disp = 0;
...
LONG RegMan_GetDWORDValue(HKEY hKey,
											   ...
											   LPDWORD pdwDest)
{
	...
	retVal = RegQueryValueEx(hKey, lpstrValueName, 0, &dwType, (LPBYTE)pdwDest, &dwcbDest);
...
LONG RegMan_SetSZValue(HKEY hKey,
											LPCTSTR lpstrValueName,
											LPCTSTR lpstrSrc)
{
	return RegSetValueEx(hKey, lpstrValueName, 0, REG_SZ, (LPBYTE)lpstrSrc, lstrlen(lpstrSrc) + 1);
}
										    ...
										    LPDWORD plcbDest)
{
	...
	retVal = RegQueryValueEx(hKey, lpstrValueName, 0, &dwType, (LPBYTE)lpstrDest, plcbDest);
...
LONG RegMan_GetNumValues(HKEY hKey,
											  LPDWORD plcValues)
{
	return RegQueryInfoKey(hKey, NULL, NULL, 0, NULL, NULL, NULL, plcValues, NULL, NULL, NULL, NULL);
}
...
{
	return RegEnumValue(hKey, dwIndex, lpstrValueName, pdwcbValueName, 0, NULL, NULL, NULL);
}
											 ...
											 LPDWORD pdwcbData)
{
	...
	retVal = RegEnumValue(hKey, dwIndex, lpstrValueName, pdwcbValueName, 0, &dwType, (LPBYTE)lpstrData, pdwcbData);
...
{
	return RegSetValueEx(hKey, lpstrValueName, 0, REG_DWORD, (LPBYTE)&dwData, sizeof(DWORD));
}
...
LONG RegMan_CopyTree(HKEY hKeySrc, HKEY hKeyDest)
{
	...
	for (i = 0; ; ++i)
	{
		...
		if (S_OK != RegEnumValue(hKeySrc, i, szName, &cbName, NULL, &dwType, rgbData, &cbData))
			break;
	...
	for (i = 0; ; ++i)
	{
		if (S_OK != RegEnumKeyEx(hKeySrc, i, szName, &cbName, NULL, NULL, NULL, NULL))
			break;
